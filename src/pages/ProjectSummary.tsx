...
const staticData: ProjectInfo[] = [
  {
    label: "Student Name",
    description: "SUBEDI RABIN",
  },
  {
    label: "Student ID",
    description: "M25w0465",
  },
  {
    label: "College Name",
    description: "The Kyoto College of Graduate Studies for Informatics",
  },
  {
    label: "Semester",
...
- Store Data in Local Storage using Zustan 
- Links to full news articles
- Smooth user experience on desktop and 
    ...
    () => [
      {
        ...
        accessorKey: "label",
        cell: ({ getValue }) => (
          <span className="font-semibold text-gray-700">{getValue<string>()}</span>
        ),
      },
      {
        header: "Description",
                ...
                {headerGroup.headers.map((header) => (
                  <th
                    ...
                    className={`border border-gray-200 px-6 py-3 text-left text-sm font-semibold text-blue-700 uppercase tracking-wide select-none
                      ${header.column.id === "label" ? "min-w-[180px] max-w-[250px]" : ""}
                    `}
                  >
                    {flexRender(
                ...
                {row.getVisibleCells().map((cell) => (
                  <td
                    ...
                    className={`border border-gray-200 px-6 py-4 text-sm align-top
                      ${cell.column.id === "label" ? "min-w-[180px] max-w-[250px] font-semibold text-gray-700" : "text-gray-600"}
                    `}
                  >
